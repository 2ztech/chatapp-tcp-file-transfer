...
void write_file(int sockfd) {
    ...
    // Read the file size
    int net_file_size;
    if (recv(sockfd, &net_file_size, sizeof(net_file_size), 0) != sizeof(net_file_size)) {
        perror("[-]Error in receiving file size.");
        return;
    }

    // Convert the file size to host byte order
    ...
    if (n > 0) {
        if (n == sizeof("EOF") && std::memcmp(buffer, "EOF", sizeof("EOF")) == 0) {
            break; // End of file
        }
